...
export default class Controller implements Routeable, Patheable {
	...
	initializeRoutes(validationProvider: Validateable) {
		...
		this.router
			.get('/image/:name', [], this.sendFile)
			.get('/image/base64/:name', [], this.sendFile64)
	...
	private sendFile64 = async (request: RequestWithUser, response: Response, next: NextFunction) => {

		let fileName: any = request.params.name

		await base64.encode(path.join(__dirname, './../../../../uploads/', fileName), (err: any, base64String: any) => {
	...
	private sendFile = async (request: RequestWithUser, response: Response, next: NextFunction) => {

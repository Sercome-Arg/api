...
export default class Controller implements Routeable, Patheable {
	...
	@inject(TYPES.Schemable) @named(TYPES.Subcategory) private subcategorySchema: Schemable
	...
	initializeRoutes(validationProvider: Validateable) {
		...
		this.router
			...
			.get(`${this.path}/search/:search`, [], this.search)
	...
	private search = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		await this.service.search(model, search)
	...
	private getAllObjs = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		if (!error) {
			await this.service.getAll(model, subcategoryModel, project, match, sort, group, limit, skip)

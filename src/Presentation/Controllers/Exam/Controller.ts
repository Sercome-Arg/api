...
import ObjInterface from '../../../Domain/Entities/Exam/Interface'
import Serviceable from '../../../Domain/Entities/Exam/Ports/Serviceable'
import QuestionServiceable from '../../../Domain/Entities/Question/Ports/Serviceable'

@injectable()
export default class Controller implements Routeable, Patheable {

	public router: Router = container.get<Router>(TYPES.Router)
	public path: string = '/exam'
	...
	@inject(TYPES.Validable) @named(TYPES.Exam) private dto: Validable
	@inject(TYPES.Schemable) @named(TYPES.Exam) private schema: Schemable
	@inject(TYPES.Schemable) @named(TYPES.Question) private questionSchema: Schemable
	@inject(TYPES.ExamServiceableDomain) private service: Serviceable
	@inject(TYPES.QuestionServiceableDomain) private questionService: QuestionServiceable
	...
	private getAllObjs = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		const questionModel: Model<Document, {}> = await this.connectionProvider.getModel(request.database, this.questionSchema.name, this.questionSchema)
			...
			await this.service.getAll(model, project, match, sort, group, limit, skip)
				.then(async (res: DomainResponseable) => {
					if(res && res.result !== undefined) {
						if(res.result) {
							if(limit === 1) {
								let exam: any = res.result
								await this.questionService.getAll(questionModel, {}, { exam: { $oid: res.result._id } }, {}, {}, 0, 0)
									.then(async (res: DomainResponseable) => {
										let questionList: any = res.result
										this.responserService.res = {
											result: {
												exam: exam,
	...
	private saveObj = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		var model: Model<Document, {}> = await this.connectionProvider.getModel(request.database, this.schema.name, this.schema)
		var questionModel: Model<Document, {}> = await this.connectionProvider.getModel(request.database, this.questionSchema.name, this.questionSchema)

		var objData: ObjInterface = request.body;
		const id = request.user._id

		let obj: any = objData
		let questionList: any[] = obj.questionList
			...
			.then((res: DomainResponseable) => {
				if(res && res.result !== undefined) {
					...
					questionList.map((question: {
						...
						question.exam = _id
						this.questionService.save(question, questionModel, id)
					})
					this.responserService.res = {
						result: res.result,
						message: res.message,

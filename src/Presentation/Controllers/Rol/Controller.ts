...
import ObjInterface from '../../../Domain/Entities/Rol/Interface'
import Serviceable from '../../../Domain/Entities/Rol/Ports/Serviceable'

@injectable()
export default class Controller implements Routeable, Patheable {

	public router: Router = container.get<Router>(TYPES.Router)
	public path: string = '/rol'
	...
	@inject(TYPES.Validable) @named(TYPES.Rol) private dto: Validable
	@inject(TYPES.Schemable) @named(TYPES.Rol) private schema: Schemable
	@inject(TYPES.Schemable) @named(TYPES.Permission) private permissionSchema: Schemable
	@inject(TYPES.RolServiceableDomain) private service: Serviceable
	...
	private getAllObjs = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		const permissionModel: Model<Document, {}> = await this.connectionProvider.getModel(request.database, this.permissionSchema.name, this.permissionSchema)
		...
		if (!error) {
			await this.service.getAll(model, permissionModel, project, match, sort, group, limit, skip)

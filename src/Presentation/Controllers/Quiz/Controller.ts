...
import ObjInterface from '../../../Domain/Entities/Quiz/Interface'
import Serviceable from '../../../Domain/Entities/Quiz/Ports/Serviceable'
import QuestionQuizServiceable from '../../../Domain/Entities/QuestionQuiz/Ports/Serviceable'

@injectable()
export default class Controller implements Routeable, Patheable {

	public router: Router = container.get<Router>(TYPES.Router)
	public path: string = '/quiz'
	...
	@inject(TYPES.Validable) @named(TYPES.Quiz) private dto: Validable
	@inject(TYPES.Schemable) @named(TYPES.Quiz) private schema: Schemable
	@inject(TYPES.Schemable) @named(TYPES.QuestionQuiz) private questionQuizSchema: Schemable
	@inject(TYPES.QuizServiceableDomain) private service: Serviceable
	@inject(TYPES.QuestionQuizServiceableDomain) private questionQuizService: QuestionQuizServiceable
	...
	private saveObj = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		var model: Model<Document, {}> = await this.connectionProvider.getModel(request.database, this.schema.name, this.schema)
		var questionQuizModel: Model<Document, {}> = await this.connectionProvider.getModel(request.database, this.questionQuizSchema.name, this.questionQuizSchema)
			...
			.then((res: DomainResponseable) => {
				if(res && res.result !== undefined) {
					...
					questionList.map((question: string) => {
						...
						this.questionQuizService.save(questionQuiz, questionQuizModel, id)
					})					
					this.responserService.res = {
						result: res.result,
						message: res.message,

...
export default class Controller implements Routeable, Patheable {
	...
	initializeRoutes(validationProvider: Validateable) {
		...
		this.router
			...
			.get(`${this.path}/by`, [], this.getByStudent)
	...
	private getByStudent = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		if(request.query !== undefined) recivedStudent = request.query.student

		await this.service.getByStudent(model, studentModel, userModel, recivedStudent)

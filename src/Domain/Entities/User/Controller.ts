...
import RegistrableWithUser from './Ports/RegistrableWithUser'
...
export default class Controller implements Serviceable {
	...
	public async getUserByEmail(
		...
		return new Promise<Responseable>( async (resolve, reject) => {
			...
			await this.geteableAllService.getAll(model, {}, { email: email, operationType: { $ne: 'D' } }, {}, {}, 1, 0)
	...
	public async getUserByUser(
		...
		return new Promise<Responseable>( async (resolve, reject) => {
			...
			await this.geteableAllService.getAll(model, {}, { user: user, operationType: { $ne: 'D' } }, {}, {}, 0, 0)
				.then((res: Responseable) => {
					if(res && res.result !== undefined) {
	...
	public async existUserWithThatUser(
		user: string,
		model: Model<Document, {}>
	): Promise<Responseable> {
	...
	public async save(
		...
		return new Promise<Responseable>( async (resolve, reject) => {
			if (idUser) {
				await this.saveableService.save(user, model, model, idUser)
					.then((res: Responseable) => {
						if(res && res.result !== undefined) {
							this.responserService = {
								result: res.result,
				...
				await this.saveableService.save(user, model, model)
					.then((res: Responseable) => {
						if(res && res.result !== undefined) {
							this.responserService = {
								result: res.result,
	...
	public async saveWithUser(
		data: RegistrableWithUser,
		model: Model<Document, {}>,
		idUser?: string
	): Promise<Responseable> {
